...
from file_utils import create_empty_dir

INPUT_DIR = "./data/tmp/UBIRIS_800_600"
...
def _extract_user_ids(input_dir: str) -> Set[int]:
    ...
    """
    set2 = set(int(user_path.split("/")[-2]) for user_path in glob(input_dir))
    print(input_dir)
    
    set1 = set(int(user_path.split("/")[-2]) for user_path in glob(input_dir))
    return set1
...
def organize_files(input_dir: str = INPUT_DIR,
    ...
    for user_id in user_ids:
        ...
        for session in (1, 2):
            ...
            for pic in pics:
                shutil.copy(pic, f"{target_user_path}_{pic_count}.jpg")
                pic_count += 1
